...
export default function AdminIssuesPage() {
  ...
  const handleDelete = async (issueId: string, title: string) => {
      ...
      <div className="min-h-screen py-8">
        <div className="container mx-auto px-4 max-w-7xl">
          ...
          <div className="card mb-6">
            <h1 className="text-2xl sm:text-3xl font-bold font-display mb-4" style={{ color: 'rgb(var(--text-primary))' }}>
              Manage Issues
            </h1>
            ...
            <div className="card overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr style={{ borderBottom: '2px solid rgb(var(--border-primary))' }}>
                    <th className="text-left py-3 px-4" style={{ color: 'rgb(var(--text-primary))' }}>Title</th>
                    <th className="text-left py-3 px-4" style={{ color: 'rgb(var(--text-primary))' }}>Category</th>
                    <th className="text-left py-3 px-4" style={{ color: 'rgb(var(--text-primary))' }}>Reporter</th>
                    <th className="text-left py-3 px-4" style={{ color: 'rgb(var(--text-primary))' }}>Status</th>
                    <th className="text-left py-3 px-4" style={{ color: 'rgb(var(--text-primary))' }}>Date</th>
                ...
                <tbody>
                  {issues.map((issue) => {
                    ...
                    return (
                      <tr key={issue._id} style={{ borderBottom: '1px solid rgb(var(--border-primary))' }}>
              ...
              {issues.length === 0 && (
                <div className="text-center py-12">
                  <p style={{ color: 'rgb(var(--text-secondary))' }}>No issues found</p>
                </div>
              )}
            </div>
          )}
